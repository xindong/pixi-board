...
export class NodeLabelLayer {
  ...
  private readonly generatingLabelIds = new Set<string>();
  ...
  tickGenerating(nowMs: number): void {
    if (this.generatingLabelIds.size === 0) return;
    for (const id of this.generatingLabelIds) {
      this.labels.get(id)?.tickGenerating(nowMs);
    }
  }
  ...
  private updateLabel(node: BoardNode, asset: Asset | undefined, scale: number): void {
    ...
    if (node.type === "generating") {
      this.generatingLabelIds.add(node.id);
    } else {
      this.generatingLabelIds.delete(node.id);
    }
  ...
  private disposeLabel(id: string): void {
    ...
    this.generatingLabelIds.delete(id);
    label.destroy();
  }
}
